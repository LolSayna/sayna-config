...
SECOND_WINDOW_CMDS=(" cd ~/Desktop/esa/CIARC/" \
...
if [ $? != 0 ]; then
    ...
    tmux send-keys -t $SESSION_NAME:2 "${STARTUP_CMDS[1]}" C-m

    tmux split-window -v -t $SESSION_NAME:2
    tmux send-keys -t $SESSION_NAME:2.2 "${SECOND_WINDOW_CMDS[0]}" C-m
    tmux send-keys -t $SESSION_NAME:2.2 "${SECOND_WINDOW_CMDS[1]}" C-m

    # Third window
    tmux new-window -t $SESSION_NAME -n "${WINDOW_NAMES[2]}"
    tmux send-keys -t $SESSION_NAME:3 "${STARTUP_CMDS[2]}" C-m
